...
class ThroughCatalog :BaseCrawler() {
    ...
    override suspend fun getPosts(doc: Document,categoryType:CategoryType): List<Post>  = coroutineScope {
        ...
        var elements = doc.select("article.tcf-article-excerpt")
        elements.forEach {
    ...
    override  fun extractImage(el: Element): String? {
        try {
            ...
            return el.select("a img[src~=(?i)\\.(png|jpe?g)]").attr("abs:src").split("?")[0]
        }catch (ex:Exception){
            ex.printStackTrace()
            return ""
        }
