...
class PickthebrainInspire :BaseCrawler() {
    ...
    override suspend fun getPosts(doc: Document,categoryType:CategoryType): List<Post>  = coroutineScope {
        ...
        var elements = doc.select("div.entry-content")
        elements.forEach {
    ...
    override  fun extractImage(el: Element): String? {
        try {
            ...
            return el.select("img[src~=(?i)\\.(png|jpe?g)]").attr("abs:src")
        }catch (ex:Exception){
            ex.printStackTrace()
            return ""
        }
